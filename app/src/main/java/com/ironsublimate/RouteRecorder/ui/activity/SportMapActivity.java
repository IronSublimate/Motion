package com.ironsublimate.RouteRecorder.ui.activity;
...
import com.ironsublimate.RouteRecorder.R;
import com.ironsublimate.RouteRecorder.commmon.bean.PathRecord;
import com.ironsublimate.RouteRecorder.commmon.bean.SportMotionRecord;
import com.ironsublimate.RouteRecorder.commmon.utils.CountTimerUtil;
import com.ironsublimate.RouteRecorder.commmon.utils.DateUtils;
import com.ironsublimate.RouteRecorder.commmon.utils.LogUtils;
import com.ironsublimate.RouteRecorder.commmon.utils.MySp;
import com.ironsublimate.RouteRecorder.commmon.utils.UIHelper;
import com.ironsublimate.RouteRecorder.db.DataManager;
import com.ironsublimate.RouteRecorder.db.RealmHelper;
import com.ironsublimate.RouteRecorder.sport_motion.MotionUtils;
import com.ironsublimate.RouteRecorder.sport_motion.PathSmoothTool;
import com.ironsublimate.RouteRecorder.ui.BaseActivity;
    ...
    private void startLocation() {
        if (mLocationClient == null) {
            ...
            mLocationOption = new AMapLocationClientOption()
                    .setLocationMode(AMapLocationClientOption.AMapLocationMode.Hight_Accuracy)//可选，设置定位模式，可选的模式有高精度、仅设备、仅网络。默认为高精度模式
                    .setGpsFirst(false)//可选，设置是否gps优先，只在高精度模式下有效。默认关闭
                    .setHttpTimeOut(30000)//可选，设置网络请求超时时间。默认为30秒。在仅设备模式下无效
                    .setInterval(interval)//可选，设置定位间隔。默认为2秒
                    .setNeedAddress(false)//可选，设置是否返回逆地理地址信息。默认是true
                    .setOnceLocation(false)//可选，设置是否单次定位。默认是false
                    .setOnceLocationLatest(false)//可选，设置是否等待wifi刷新，默认为false.如果设置为true,会自动变为单次定位，持续定位时不要使用

                    .setSensorEnable(false)//可选，设置是否使用传感器。默认是false
                    .setWifiScan(true) //可选，设置是否开启wifi扫描。默认为true，如果设置为false会同时停止主动刷新，停止以后完全依赖于系统刷新，定位位置可能存在误差
                    .setLocationCacheEnable(true) //可选，设置是否使用缓存定位，默认为true
                    .setGeoLanguage(AMapLocationClientOption.GeoLanguage.ZH);//可选，设置逆地理信息的语言，默认值为默认语言（根据所在地区选择语言）
